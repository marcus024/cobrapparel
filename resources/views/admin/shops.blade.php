...
{{-- Edit Modal --}}
<div id="editShopModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-5 rounded-lg shadow-lg w-[350px]">
        <h2 class="text-xl font-bold mb-4">Edit Shop</h2>
        <input type="text" id="editShopName" class="border p-2 w-full mb-3" placeholder="Shop Name">
        <input type="text" id="editShopOwner" class="border p-2 w-full mb-3" placeholder="Owner">
        <input type="text" id="editContactName" class="border p-2 w-full mb-3" placeholder="Contact Name">
        <input type="tel" id="editContactNumber" class="border p-2 w-full mb-3" placeholder="Contact Number">
        <input type="email" id="editEmailAddress" class="border p-2 w-full mb-3" placeholder="Email Address">
        <label class="border p-2 w-full mb-3 flex justify-start items-center cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-md">
            Shop Duration
        </label>
        <input type="date" id="editShopDuration" class="border p-2 w-full mb-3">

        <div class="relative w-full">
            <input type="file" id="editShopImage" class="hidden" accept="image/*" onchange="previewEditImage(event)">
            <label for="editShopImage" class="border p-2 w-full mb-3 flex justify-center items-center cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-md">
                Change Shop Logo
            </label>
            <div id="editPreviewContainer" class="mt-3 hidden">
                <img id="editPreviewImage" src="" alt="Shop Logo" class="w-24 h-24 object-cover rounded-md border">
            </div>
        </div>

        <button class="bg-[#700101] mt-4 text-white px-4 py-2 rounded" onclick="updateShop()">Save Changes</button>
        <button class="bg-gray-500 text-white px-4 py-2 rounded ml-2" onclick="document.getElementById('editShopModal').classList.add('hidden')">Cancel</button>
    </div>
</div>
@endsection
